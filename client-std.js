...
`
    ...
    function SettingsPage(p) {
      ...
      function doExport() {
        fetch('/api/workbench/export', { method: 'POST' }).then((r) => r.json()).then((r) => { setMsg(r && r.ok ? '已导出到 ' + (r.path || '工作区') : '导出失败') }).catch(() => setMsg('导出失败'))
      }
      function doReset() {
        fetch('/api/workbench/reset', { method: 'POST' }).then((r) => r.json()).then(() => { setMsg('已重置为默认数据'); p.update(() => {}) }).catch(() => setMsg('重置失败'))
      }
    ...
    function Workbench(props) {
      ...
      React.useEffect(() => {
        let alive = true
        fetch('/api/workbench/state').then((r) => r.json()).then((d) => { if (alive) setData(normalize(d)) }).catch(() => { if (alive) setData(normalize(null)) })
        return () => { alive = false }
      }, [])
      function update(fn) {
        setData((prev) => {
          ...
          fetch('/api/workbench/save', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(norm) }).catch(() => {})
          return norm
        })
      }
