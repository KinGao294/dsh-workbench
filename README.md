...
- **看板首页**：早安横幅（问候语 + 日期 + 每日一句）、KPI 卡片、今日日程、最新 AI 简报、今日习惯、今日完成清单（快速记录）、昨日补记、本月预算、目标进度、待办任务
- **12 个模块全部可用**：
  - 看板（总览）/ 热点简报（收录 AI 热点）/ 工作日程 / 开销记账 / 选题库（待写→写作中→已发布）/ 读书管理（想读→在读→读完 + 进度）/ 习惯打卡 / 目标管理（进度滑块）/ 自动复盘（一键生成）/ 任务中心（优先级排序）/ 完成清单 / 设置（名称 + 4 主题 + 导出 + 重置）
